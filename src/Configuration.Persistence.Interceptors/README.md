...
> [!NOTE]
> `AuditTrailSaveChangesInterceptor<TAuditRecord, TContext>` holds entries whose keys are generated by the store between saving and saved, keyed by the `DbContext` that is saving them. A single instance can therefore be shared by every context in the application, which is what `AddInterceptors` does. A save that fails discards only its own held back entries, so it never contributes a trail entry for a row that was rolled back.
